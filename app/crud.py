...
def create_product(
    db: Session, product: schemas.ProductBase, seller_id: int = None
) -> models.Product:
...
def update_product(
    db: Session, product_id: int, update: schemas.ProductBase, seller_id: int = None
...
def delete_product(
    db: Session, product_id: int, user_id: int = None, user_role: str = None
) -> Optional[models.Product]:
    product = db.query(models.Product).filter(models.Product.id == product_id).first()
    if not product:
        return None
...
def add_cart_item(
    db: Session, cart_id: int, item: schemas.CartItemBase
) -> models.CartItem:
...
def create_order(
    ...
    user_id: int,
    items: List[schemas.OrderItemBase],
) -> models.Order:
    db_order = models.Order(
        user_id=user_id, address_id=order.address_id, total_amount=order.total_amount
    )
    db.add(db_order)
    db.commit()
    db.refresh(db_order)
...
def update_address(
    db: Session, db_address: models.Address, update: schemas.AddressUpdate
):
    for key, value in update.dict(exclude_unset=True).items():
...
def add_to_wishlist(db: Session, user_id: int, product_id: int) -> models.Wishlist:
    exists = (
        ...
        .filter(
            models.Wishlist.user_id == user_id, models.Wishlist.product_id == product_id
        )
        .first()
    )
    if exists:
...
# SHIPMENTS
def create_shipment(
    db: Session, shipment_in: schemas.ShipmentCreate
) -> models.Shipment:
    order = (
        db.query(models.Order).filter(models.Order.id == shipment_in.order_id).first()
    )
    if not order:
